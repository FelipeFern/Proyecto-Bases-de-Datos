...
create procedure reservaVueloIda(IN numero INT, IN clase VARCHAR(20), IN fecha DATE, IN tipo_doc VARCHAR(5), IN numero_doc INT, IN legajo INT, OUT resultado CHAR(100))
begin	
	select numero as Numero, clase as Clase, fecha as Fecha, tipo_doc as TipoDoc, numero_doc as NumeroDoc, legajo as Legajo;	
	...
	SELECT b.cant_asientos INTO asientosBrindados FROM brinda as b WHERE b.vuelo = numero and b.clase = clase ; 
	SELECT count(*) INTO asientosReservados FROM reserva_vuelo_clase rvc WHERE rvc.vuelo = numero AND rvc.fecha_vuelo = fecha;
	
	# Deberia crear una reserva y crear asientos_reservados y nada mas???
	
	if (SELECT count(*) FROM empleados as e WHERE e.legajo = legajo) = 0 then
		 SET resultado = 'El empleado ingresado no existe';
	else 
		if (SELECT count(*) FROM instancias_vuelo as iv WHERE fecha = iv.fecha AND iv.vuelo = numero) = 0 then
			SET resultado = 'El vuelo ingresado no existe';
		else 
			if (SELECT count(*) FROM pasajeros as p WHERE p.doc_tipo = tipo_doc AND p.doc_nro = numero_doc) = 0 then
			SET resultado = 'El pasajero no existe';
				...
				if asientos_disp < 0 then
					...
					# Ingreso la reserva en reservas
						...
						VALUES (numeroReserva,tipo_doc , numero_doc, legajo, fecha , DATE_ADD(NOW(), INTERVAL 15 day), estado);
